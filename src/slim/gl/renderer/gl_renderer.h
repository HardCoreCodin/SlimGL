...
#include "gl_text_pass.h"
#include "gl_full_screen_image_pass.h"
...
namespace gl {
	...
	void main()
	{
		...
		void init(Scene &main_scene, CubeMapSet *cube_map_sets, u32 cube_map_sets_count, RawImage *texture_images, u32 texture_count, Text *static_texts = nullptr, u32 static_texts_count = 0, bool wireframe = false, bool normals = false) {
			...
			if (static_texts && static_texts_count) {
				texts_count = static_texts_count;
				texts = new GLText[texts_count];
				for (u32 i = 0; i < static_texts_count; i++) {
					texts[i].create();
                    texts[i].update(static_texts[i]);
				}
			}
